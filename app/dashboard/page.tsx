...
export default function Dashboard() {
  ...
  async function refreshProposalStatuses(records: RfpRecord[]) {
    ...
    return records.map(item => ({
      ...item,
      proposal_state: item.analysis_id && ready.has(item.analysis_id) ? "ready" : item.analysis_id && paid.has(item.analysis_id) ? "paid" : "unpaid",
    }));
  }
  ...
  async function analyzeRfp() {
    ...
    try {
      ...
      const analyzing = { ...saved, status: "analyzing", updated_at: new Date().toISOString(), analysis_id: null, proposal_state: "unpaid" as ProposalState };
  ...
  async function retryAnalysis(item: RfpRecord) {
    if (busy) return;
    setBusy(true);
    const analyzing = { ...item, status: "analyzing", analysis_id: null, proposal_state: "unpaid" as ProposalState, updated_at: new Date().toISOString() };
